...
git fetch origin
# A branch created from a newer branch than dev (e.g. stage while dev lags) would
...
git merge-base --is-ancestor "origin/$BASE_BRANCH" "$branch" \
  || die "origin/$BASE_BRANCH is not an ancestor of $branch. Fast-forward $BASE_BRANCH first (e.g. git push origin stage:$BASE_BRANCH), then re-run"
