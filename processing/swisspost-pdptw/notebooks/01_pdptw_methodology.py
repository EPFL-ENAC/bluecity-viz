...
@app.cell
def _(itertools, np):
    locations = {'hub': (0.0, 0.0), 'A': (2.0, 8.0), 'B': (7.0, 6.0), 'C': (8.0, 1.0), 'D': (3.0, 3.0)}
    requests = [('hub', 'A', 4), ('B', 'C', 6), ('hub', 'D', 3)]
    CAPACITY = 10
    SERVICE_MIN = 2.0
    START_MIN = 0.0
    # Time windows per task (None = always open). Kept immutable — the
    # what-if experiments below build modified *copies*.
    time_windows = {('P', 0): None, ('D', 0): None, ('P', 1): None, ('D', 1): None, ('P', 2): None, ('D', 2): None}

    def phy(task) -> str:
        kind, i = task
        return requests[i][0] if kind == 'P' else requests[i][1]

    def simulate(order, locs, tws) -> dict | None:
        """Drive the task sequence on the map `locs`; None if infeasible."""

        def travel(a: str, b: str) -> float:
            (x1, y1), (x2, y2) = locs[a], locs[b]
        ...
        for task in order:
            ...
            t = t + SERVICE_MIN
            load = load + requests[task[1]][2] * (1 if task[0] == 'P' else -1)
            if load > CAPACITY:
    ...
    def all_feasible_orders():
        ...
        tasks = [('P', i) for i in range(len(requests))] + [('D', i) for i in range(len(requests))]
        for perm in itertools.permutations(tasks):
            if all((perm.index(('P', i)) < perm.index(('D', i)) for i in range(len(requests)))):
                yield perm

    def brute_force(locs, tws):
        """All feasible tours on (locs, tws), best first."""
        res = []
        for order in all_feasible_orders():
            sim = simulate(order, locs, tws)
            if sim is not None:
                res.append((sim['drive'], order))
        return sorted(res, key=lambda r: r[0])

    results = brute_force(locations, time_windows)
    ...
    return best_drive, best_order, brute_force, locations, phy, time_windows
...
@app.cell
def _(best_drive, best_order, locations, phy, plt):
...
@app.cell
def _(brute_force, locations, time_windows):
    # Give request 1's delivery a window that is comfortable at speed x1.0,
    # then uniformly slow the whole map down (scaling coordinates scales all
    # euclidean travel times).
    tw_tight = {**time_windows, ('D', 1): (14.0, 20.0)}

    for scale in (1.0, 1.15, 1.4):
        scaled = {n: (sx * scale, sy * scale) for n, (sx, sy) in locations.items()}
        res = brute_force(scaled, tw_tight)
        if not res:
