...
def _():
    ...
    return M_SP, config, json, matrixtools, nodes, np, osm, pd, plt
...
@app.cell
def _(mo):
    # Point this at your own matrix file to use it instead of the OSMnx build
    osm_path_ui = mo.ui.text(
        label="OSM matrix path (optional — .npy or CSV, see data/README.md)",
        value="",
        full_width=True,
    )
    osm_path_ui
...
@app.cell
def _(M_SP, nodes, np, osm, osm_path_ui, pd):
    OSM_MATRIX_PATH = osm_path_ui.value.strip() or None
    ...
    return M_OSM, snaps, source, tag, truth
...
@app.cell
def _(M_OSM, M_SP, matrixtools, nodes, np):
...
def _(M_OSM, M_SP, np, plt, tag):
    ...
    print(f'ratio SP/OSM: median {np.median(ratio):.3f}, geometric mean {np.exp(np.mean(np.log(ratio))):.3f}, 5–95% [{np.quantile(ratio, 0.05):.2f}, {np.quantile(ratio, 0.95):.2f}]')
    _fig, _axes = plt.subplots(1, 2, figsize=(11, 4.5))
    _axes[0].scatter(x / 60, y / 60, s=6, alpha=0.3)
    lim = max(x.max(), y.max()) / 60 * 1.02
    _axes[0].plot([0, lim], [0, lim], 'k--', lw=1, label='y = x')
    _axes[0].plot([0, lim], [0, lim * np.median(ratio)], 'r-', lw=1, label=f'y = {np.median(ratio):.2f}·x (median ratio)')
    _axes[0].set_xlabel('OSM (min)')
    _axes[0].set_ylabel('Swiss Post (min)')
    _axes[0].set_title(f'OD travel times{tag}')
    _axes[0].legend()
    _axes[1].hist(ratio, bins=60)
    _axes[1].axvline(1, c='k', ls='--', lw=1)
    _axes[1].axvline(np.median(ratio), c='r', lw=1)
    _axes[1].set_xlabel('ratio SP / OSM')
    _axes[1].set_title(f'Ratio distribution{tag}')
    plt.tight_layout()
    _fig
    return n, ratio, valid, x, y
...
def _(matrixtools, x, y):
    ...
    k_hat = fo["k"]
    return af, bp, fo, k_hat, ll


@app.cell
def _(k_hat, np, plt, ratio, tag, x):
    ...
    for b in range(12):
        ...
        if len(r) >= 10:
            ...
            q10.append(np.quantile(r, 0.1))
            q50.append(np.quantile(r, 0.5))
            q90.append(np.quantile(r, 0.9))
    _fig, _ax = plt.subplots(figsize=(8, 4.5))
    _ax.fill_between(centers, q10, q90, alpha=0.25, label='10–90% band')
    _ax.plot(centers, q50, '-o', label='median ratio')
    _ax.axhline(k_hat, c='r', ls='--', lw=1, label=f'k̂ = {k_hat:.2f}')
    _ax.axhline(1, c='k', lw=0.5)
    _ax.set_xlabel('OSM travel time (min)')
    _ax.set_ylabel('ratio SP / OSM')
    _ax.set_title(f'Is the factor constant across trip lengths?{tag}')
    _ax.legend()
    plt.tight_layout()
    _fig
    return
...
@app.cell
def _(M_OSM, M_SP, k_hat, nodes, np, plt, tag, valid):
    ...
    for _ax, vals, ttl in [(_axes[0], per_origin, 'as origin'), (_axes[1], per_dest, 'as destination')]:
        sc = _ax.scatter(nodes.lon, nodes.lat, c=vals, cmap='coolwarm', vmin=0.8, vmax=1.2, s=60)
        _ax.set_title(f'mean ratio / k̂, {ttl}{tag}')
        _ax.set_aspect(1 / np.cos(np.radians(nodes.lat.mean())))
    _fig.colorbar(sc, ax=_axes, shrink=0.8, label='ratio relative to k̂ (1 = perfect)')
    _fig
    return (resid_ratio,)


@app.cell
def _(M_OSM, M_SP, np, plt, tag, valid):
    ...
    _ax.scatter(r_fwd, r_bwd, s=5, alpha=0.3)
    _ax.plot([r_fwd.min(), r_fwd.max()], [r_fwd.min(), r_fwd.max()], 'k--', lw=1)
    _ax.set_xlabel('ratio i→j')
    _ax.set_ylabel('ratio j→i')
    _ax.set_title(f'Direction dependence of the ratio{tag}')
    plt.tight_layout()
    dir_corr = float(np.corrcoef(r_fwd, r_bwd)[0, 1])
    print(f'corr(r_ij, r_ji) = {dir_corr:.3f} (high = direction-neutral error; low = direction-dependent)')
    _fig
    return (dir_corr,)
...
@app.cell
def _(M_OSM, M_SP, hav, n, np, pd, resid_ratio, snaps):
    logr = np.abs(np.log(resid_ratio))
    flat = np.argsort(np.nan_to_num(logr, nan=-1).ravel())[::-1][:20]
    rows = []
    for _f in flat:
        i, j = divmod(_f, n)
        row = {'from': i, 'to': j, 'hav_km': hav[i, j] / 1000, 'osm_min': M_OSM[i, j] / 60, 'sp_min': M_SP[i, j] / 60, 'ratio': M_SP[i, j] / M_OSM[i, j]}
...
@app.cell
def _(M_OSM, M_SP, matrixtools, np):
...
def _(
    ...
    if abs(ll['slope_b'] - 1) < 0.05 and fo['r2'] > 0.9:
        shape = 'uniform'
        verdict = f"UNIFORM factor: SP ≈ {k_hat:.2f} × OSM explains the gap (R²={fo['r2']:.3f}). Methodology sound — apply k, then re-check time-window feasibility (notebook 01 §5)."
    elif af['r2'] > 0.95 and abs(ll['slope_b'] - 1) < 0.1:
        shape = 'affine'
        verdict = f"AFFINE, near-uniform: SP ≈ {af['k']:.2f} × OSM + {af['c']:.0f} s (R²={af['r2']:.3f}). Methodology sound with a TWO-parameter calibration: a multiplicative factor (congestion & speed limits) plus a fixed per-trip overhead (junction/access time). A single scalar over-corrects long trips and under-corrects short ones."
    else:
        shape = 'structural'
        verdict = 'STRUCTURAL differences beyond a scalar — inspect §3 (distance dependence), §4 (geography/direction), §5 (snapping) for which kind.'
    print(f'[{source}] {verdict}')
    summary = {'source': source, 'k_through_origin': fo['k'], 'r2_through_origin': fo['r2'], 'affine_k': af['k'], 'affine_c_s': af['c'], 'loglog_slope': ll['slope_b'], 'loglog_r2': ll['r2'], 'breusch_pagan_p': bp['p_value'], 'ratio_median': float(np.median(ratio)), 'ratio_q05': float(np.quantile(ratio, 0.05)), 'ratio_q95': float(np.quantile(ratio, 0.95)), 'direction_corr': dir_corr, 'permutation_suspect': perm['suspect_permutation'], 'gap_shape': shape, 'verdict': verdict, 'planted_truth': truth}
    with open(config.OUTPUTS_DIR / 'summary_od_comparison.json', 'w') as _f:
