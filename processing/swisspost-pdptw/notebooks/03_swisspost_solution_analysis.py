...
@app.cell
def _(sol, solution, uuid_to_idx):
...
@app.cell
def _(M, legs, np):
    legs_check = legs.assign(
        matrix_s=[M[int(i), int(j)] for i, j in zip(legs.from_idx, legs.to_idx)]
    )
    exact = (legs_check.seconds == legs_check.matrix_s).mean()
    print(f"legs matching the matrix exactly: {exact:.1%}  "
          f"(n = {len(legs_check)}, correlation = "
          f"{np.corrcoef(legs_check.seconds, legs_check.matrix_s)[0, 1]:.6f})")
...
def _(pd, plt, tours):
    ...
    _ax.bar(t.index, t.drive_min, label='drive')
    _ax.bar(t.index, t.service_min, bottom=t.drive_min, label='service')
    _ax.bar(t.index, t.wait_min, bottom=t.drive_min + t.service_min, label='wait')
    _ax.set_xlabel('tour (sorted by duration)')
    _ax.set_ylabel('minutes')
    _ax.set_title('All 87 tours: total duration = drive + service + wait')
    _ax.legend()
    plt.tight_layout()
    _fig
    return
...
@app.cell
def _(extraction, nodes, plt, tours):
    vt = extraction.get_visit_times(nodes)
    service_pred_min = tours.stops.apply(
        lambda s: sum((vt.get(x, 0) for x in s[1:-1])) / 60
    )
    resid = (tours.service_min - service_pred_min).abs()
    print(f'median |difference|: {resid.median():.2f} min; tours within 1 min: {(resid < 1).mean():.0%}')

    _fig, _ax = plt.subplots(figsize=(5.5, 5))
    _ax.scatter(service_pred_min, tours.service_min, s=25)
    lim = max(tours.service_min.max(), service_pred_min.max()) * 1.05
    _ax.plot([0, lim], [0, lim], 'k--', lw=1, label='y = x')
    _ax.set_xlabel('Σ visit_time over stops (min)')
    _ax.set_ylabel('observed service in solution (min)')
    _ax.set_title('Service time: solution vs instance visit_time')
    _ax.legend()
    plt.tight_layout()
    _fig
    return
...
def _(plt, tours):
    ...
    for _ax, col in zip(axes, ['total_min', 'drive_min', 'n_stops']):
        _ax.hist(tours[col], bins=25)
        _ax.set_title(col)
...
@app.cell
def _(area_tours, config, nodes, np, plt):
    ...
    _ax.scatter(area_nodes.lon, area_nodes.lat, c='lightgray', s=35, zorder=1)
    hub = area_nodes[area_nodes.is_hub].iloc[0]
    _ax.scatter(hub.lon, hub.lat, marker='s', c='black', s=90, zorder=3, label='hub')
    for _, tour in area_tours.head(6).iterrows():
        pts = [idx_to_ll[s] for s in tour.stops if s in idx_to_ll]
        _ax.plot([p[1] for p in pts], [p[0] for p in pts], '-o', ms=3, lw=1.2, alpha=0.75, zorder=2, label=str(tour.date))
    _ax.set_title(f'First tours of area {config.AREA_ID} (straight-line rendering)')
    _ax.legend(fontsize=8)
    _ax.set_aspect(1 / np.cos(np.radians(area_nodes.lat.mean())))
    plt.tight_layout()
    _fig
    return
...
@app.cell
def _(area_tours, config, json, tours):
    benchmark = {
