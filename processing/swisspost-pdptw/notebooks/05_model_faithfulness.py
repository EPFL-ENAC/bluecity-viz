...
def _():
    ...
    with open(config.OUTPUTS_DIR / 'benchmark_tours.json') as _f:
        benchmark = json.load(_f)
    tours = pd.DataFrame(benchmark['tours'])
    with open(config.OUTPUTS_DIR / 'summary_od_comparison.json') as _f:
        od = json.load(_f)
    k_hat, c_hat = (od['affine_k'], od['affine_c_s'])
    print(f"{len(tours)} benchmark tours | OD calibration: SP ≈ {k_hat:.3f}·OSM + {c_hat:.0f}s  [{od['source']}]")
    ...
    M_OSM_cal = k_hat * M_OSM_f + c_hat
    np.fill_diagonal(M_OSM_cal, 0.0)  # affine-calibrated OSM
    return (
        M_OSM_cal,
        M_OSM_f,
        M_SP,
...
@app.cell
def _(M_OSM_cal, M_OSM_f, M_SP, evaluate, np, pd, tours, visit_times):
    scenarios = {'baseline': (M_SP, visit_times), 'service_5min': (M_SP, 300.0), 'osm': (M_OSM_f, visit_times), 'osm_calibrated': (M_OSM_cal, visit_times)}
    evals = {_name: evaluate.evaluate_tours(tours, m, vt, label=_name) for _name, (m, vt) in scenarios.items()}
    assert np.allclose(evals['baseline'].drive_min, tours.drive_min, atol=0.01), 'baseline re-evaluation failed to reproduce the benchmark'
    print('baseline reproduces every benchmark tour exactly ✓')
    rows = []
    for _name, ev in evals.items():
        rows.append({'scenario': _name, 'drive_min_total': ev.drive_min.sum(), 'service_min_total': ev.service_min.sum(), 'total_min': ev.total_min.sum(), 'vs_benchmark_pct': 100 * (ev.total_min.sum() / evals['baseline'].total_min.sum() - 1)})
...
@app.cell
def _(evals, osm_source, plt):
    base_drive = evals['baseline'].drive_min
    _fig, _ax = plt.subplots(figsize=(9, 4.5))
    for _name, color in [('osm', 'tab:red'), ('osm_calibrated', 'tab:green')]:
        delta = evals[_name].drive_min - base_drive
        _ax.scatter(base_drive, delta, s=14, alpha=0.6, c=color, label=_name)
    _ax.axhline(0, c='k', lw=0.8)
    _ax.set_xlabel('benchmark tour drive (min)')
    _ax.set_ylabel('drive difference vs benchmark (min)')
    _ax.set_title(f'Per-tour drive error of the OSM matrix, before/after calibration [{osm_source}]')
    _ax.legend()
    plt.tight_layout()
...
def _(evals, plt):
    ...
    _ax.hist(sv_err, bins=30)
    _ax.axvline(0, c='k', lw=0.8)
    _ax.set_xlabel('service-time error per tour (min), uniform 5min − real visit_time')
    _ax.set_title("What 'service = 5 min per stop' does to each tour")
    plt.tight_layout()
    print(f'per-tour service error: mean {sv_err.mean():+.1f} min, range [{sv_err.min():.0f}, {sv_err.max():.0f}] min')
    _fig
    return
...
@app.cell
def _(extraction, instance, nodes):
...
def _(
    ...
    HUB_IDX = int(nodes[nodes.is_hub & nodes.in_areas.apply(lambda a: config.AREA_ID in a)]['index'].iloc[0])

    def solve_day(date: str, matrix: np.ndarray, seed: int=0, runtime_s: int=3) -> dict:
        ...
        m.add_vehicle_type(num_available=3, capacity=int(1039000), tw_early=420 * 60, tw_late=1020 * 60, shift_duration=6 * 3600, start_depot=depot, end_depot=depot)
        for _, s in day.iterrows():  # position -> matrix index
            loc = int(s.to_idx if s.to_idx != HUB_IDX else s.from_idx)
            is_delivery = s.to_idx != HUB_IDX
            clients_idx.append(loc)
            m.add_client(x=len(clients_idx), y=0, delivery=int(s.weight) if is_delivery else 0, pickup=0 if is_delivery else int(s.weight), service_duration=int(visit_times.get(loc, 0)), tw_early=int(s.dropoff_tw[0] * 60), tw_late=int(s.dropoff_tw[1] * 60))
        locs = [HUB_IDX] + clients_idx
        for a, fa in enumerate(m.locations):
            for b, fb in enumerate(m.locations):  # the instance's 6-hour driver limit
                if a != b:
        ...
        return {'objective': res.cost(), 'arcs': arcs, 'feasible': res.best.is_feasible()}

    def jaccard(a: set, b: set) -> float:
        return len(a & b) / len(a | b) if a | b else 1.0

    # ~28 PyVRP solves (~90 s). Disk-cached: re-runs are instant unless the
    # matrices or this cell's code change.
    with mo.persistent_cache(name="pyvrp_solves"):
        records = []
        for date in DATES:
            sp0 = solve_day(date, M_SP, seed=0)
            noise = [jaccard(sp0['arcs'], solve_day(date, M_SP, seed=s)['arcs']) for s in (1, 2, 3)]
            osm_sol = solve_day(date, M_OSM_f, seed=0)
            cal_sol = solve_day(date, M_OSM_cal, seed=0)
            records.append({'date': date, 'sp_obj_min': sp0['objective'] / 60, 'osm_obj_min': osm_sol['objective'] / 60, 'cal_obj_min': cal_sol['objective'] / 60, 'overlap_noise_baseline': np.mean(noise), 'overlap_sp_vs_osm': jaccard(sp0['arcs'], osm_sol['arcs']), 'overlap_sp_vs_cal': jaccard(sp0['arcs'], cal_sol['arcs'])})
    solves = pd.DataFrame(records)
    print(solves.round(3).to_string(index=False))
    print(f'\nobjective ratio sp/osm: {(solves.sp_obj_min / solves.osm_obj_min).mean():.3f} (matrix k̂ = {k_hat:.3f})')
    return (solves,)
...
def _(
    ...
    gap_service = float(evals['service_5min'].total_min.sum() / evals['baseline'].total_min.sum() - 1)
    summary = {'osm_source': osm_source, 'matrix_gap_drive_pct': gap_matrix * 100, 'matrix_gap_after_calibration_pct': gap_matrix_after_cal * 100, 'calibration': {'k': k_hat, 'c_s': c_hat, 'shape': od['gap_shape']}, 'service_assumption_total_pct': gap_service * 100, 'tw_bug_shipments_affected': int(len(tw)), 'tw_real_width_min': 30, 'solver_noise_route_overlap': float(solves.overlap_noise_baseline.mean()), 'sp_vs_osm_route_overlap': float(solves.overlap_sp_vs_osm.mean()), 'sp_vs_calibrated_route_overlap': float(solves.overlap_sp_vs_cal.mean())}
    with open(config.OUTPUTS_DIR / 'summary_faithfulness.json', 'w') as _f:
